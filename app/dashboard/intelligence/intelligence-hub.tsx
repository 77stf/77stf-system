...
function ContentScout() {
  ...
  const isVideo = contentType === 'youtube_video' || contentType === 'instagram_post'
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <p style={{ color: t.text.muted, fontSize: 13, marginBottom: 20, lineHeight: 1.6 }}>
        Wklej link do YouTube, Instagram, artykułu lub tekst. Scout pobiera transkrypt wideo i przeprowadza pełną analizę pod 77STF.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
        <textarea
          ...
          rows={4}
          placeholder="https://youtube.com/watch?v=... lub https://www.instagram.com/reel/... lub tekst artykułu..."
          style={{
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            disabled={analyzing || content.trim().length < 3}
            style={{
              ...
              cursor: (analyzing || content.trim().length < 3) ? 'not-allowed' : 'pointer',
              opacity: (analyzing || content.trim().length < 3) ? 0.6 : 1,
            }}
          >
            <Zap size={14} />
            {analyzing ? (isVideo ? 'Pobieram transkrypt...' : 'Analizuję...') : 'Analizuj (Scout)'}
          </button>
          {result && (
            <button
